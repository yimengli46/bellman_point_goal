...
from modeling.utils.map_utils_egocentric_to_initial import SemanticMap_Egocentric
...
cfg.merge_from_file(
    'configs/exp_90degree_Optimistic_PCDHEIGHT_MAP_1STEP_500STEPS.yaml')
cfg.freeze()
...
allo_SemMap = SemanticMap(split, scene_name, pose_range, coords_range, WH,
                            ins2cat_dict)  # build the observed sem map
ego_SemMap = SemanticMap_Egocentric(split, scene_name, pose_range, coords_range, WH, ins2cat_dict)
...
if cfg.NAVI.HFOV == 90:
    obs_list, rel_pose_list, allo_pose_list = [], [], []
    heading_angle = start_pose[2]
    obs, rel_pose, allo_pose = get_obs_and_pose(env, agent_pos, heading_angle)
    obs_list.append(obs)
    rel_pose_list.append(rel_pose)
    allo_pose_list.append(allo_pose)
...
while step < cfg.NAVI.NUM_STEPS:
    ...
    allo_pose = allo_pose_list[-1]
    #print(f'agent position = {pose[:2]}, angle = {pose[2]}')
    agent_map_pose = (allo_pose[0], -allo_pose[1], -allo_pose[2])
    ...
    t0 = timer()
    allo_SemMap.build_semantic_map(obs_list,
                                     allo_pose_list,
                                     step=step,
                                     saved_folder=saved_folder)
    #'''
    ego_SemMap.build_semantic_map(obs_list, rel_pose_list, step=step, saved_folder=saved_folder)
    ego_built_semantic_map, ego_observed_area_flag, ego_occupancy_map = ego_SemMap.get_semantic_map()
    ego_built_semantic_map = ego_built_semantic_map[50:-50, 50:-50]
    color_ego_built_semantic_map = apply_color_to_map(ego_built_semantic_map, flag_small_categories=True)

    plt.imshow(color_ego_built_semantic_map) #, cmap='gray')
    plt.show()
    #'''
    t1 = timer()
    ...
    if MODE_FIND_SUBGOAL:
        t1 = timer()
        observed_occupancy_map, gt_occupancy_map, observed_area_flag, built_semantic_map = allo_SemMap.get_observed_occupancy_map(
            agent_map_pose)
        t2 = timer()
    ...
    if True:
        ...
        #built_semantic_map, observed_area_flag, _ = allo_SemMap.get_semantic_map()
    ...
    if act == -1 or act == 0:  # finished navigating to the subgoal
        ...
        if cfg.NAVI.HFOV == 90:
            obs_list, rel_pose_list, allo_pose_list = [], [], []
            obs, rel_pose, allo_pose = get_obs_and_pose_by_action(env, act, allo_pose)
            obs_list.append(obs)
            rel_pose_list.append(rel_pose)
            allo_pose_list.append(allo_pose)
