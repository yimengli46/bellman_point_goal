...
def find_neighborhood(agent_coords, occupancy_map):
    radius = int(cfg.SENSOR.SENSOR_HEIGHT /
                 cfg.SEM_MAP.CELL_SIZE) + cfg.SEM_MAP.GAP_COMPLEMENT
    rr_cir, cc_cir = circle_perimeter(agent_coords[1],
                                      agent_coords[0],
                                      radius,
                                      method='andres')
    ...
    for idx in range(len(rr_cir)):
        rr_line, cc_line = line(agent_coords[1], agent_coords[0], rr_cir[idx],
                                cc_cir[idx])
...
class SemanticMap_Egocentric:
    ...
    def __init__(self, split, scene_name, pose_range, coords_range, WH,
        ...
        if True:  #cfg.NAVI.GT_OCC_MAP_TYPE == 'NAV_MESH':
            ...
            gt_occupancy_map = np.where(gt_occupancy_map == 0,
    ...
    def build_semantic_map(self, obs_list, pose_list, step=0, saved_folder=''):
        ...
        #========================== Transform old point cloud to new view ==================
        with torch.no_grad():
            tensor_map = torch.tensor(self.four_dim_grid).permute(0, 2, 1, 3) # z, x, y, c
            r, c = tensor_map.shape[:2]
            tensor_map = tensor_map.reshape((r, c, -1)) # z, x, yxc
            tensor_map = tensor_map.unsqueeze(0).permute(0, 3, 1, 2).float() # B, yxc, z, x 
            size = tensor_map.shape

            pose = pose_list[0]
            tx, tz, theta = pose[0], -pose[1], -pose[2]
            print(f'============> tx = {tx}, tz = {tz}, theta = {theta}')
            angle = -theta 
            tx = tx
            tz = -tz
            translation = np.array([tx/12.0, tz/12.0])
            ...
            output_map = F.grid_sample(tensor_map, grid, mode='nearest', align_corners=True)
            output_map = output_map.permute(0, 2, 3, 1).squeeze(0).reshape((r, c, len(self.y_grid)+1, cfg.SEM_MAP.GRID_CLASS_SIZE)) # z, x, y, c
            output_map = output_map.permute(0, 2, 1, 3).numpy()

        self.four_dim_grid = output_map
        ...
        for idx, obs in enumerate(obs_list):
            ...
            if cfg.SEM_MAP.FLAG_VISUALIZE_EGO_OBS and cfg.NAVI.HFOV == 90:
                ...
                ax[0].set_title("rgb")
                ax[1].imshow(
                    apply_color_to_map(sseg_img, flag_small_categories=True))
            ...
            #'''
            if cfg.NAVI.HFOV == 90:
                xyz_points, sseg_points = project_pixels_to_camera_coords(
                    sseg_img,
                    depth_img,
                    gap=2,
                    FOV=90,
    ...
    def get_semantic_map(self):
        ...
        mask_occupied = np.logical_and(
            cells_in_occupied_range >= cfg.SEM_MAP.POINTS_CNT,
            observed_area_flag)
    ...
    def get_observed_occupancy_map(self, agent_map_pose):
        ...
        self.neighborhood_history = np.logical_or(self.neighborhood_history,
                                                  neighborhood_mask)
        complement_mask = np.logical_and(
            self.neighborhood_history, occupancy_map == cfg.FE.UNOBSERVED_VAL)
        # change the complement area
        if cfg.NAVI.GT_OCC_MAP_TYPE == 'PCD_HEIGHT':
            occupancy_map = np.where(complement_mask, cfg.FE.FREE_VAL,
                                     occupancy_map)
        elif cfg.NAVI.GT_OCC_MAP_TYPE == 'NAV_MESH':
