...
class Data_Gen_View:
    ...
    def init_scene(self):
        ...
        self.LS = localNav_slam(self.pose_range,
                                self.coords_range,
                                self.WH,
                                mark_locs=True,
        ...
        self.gt_occupancy_map = np.where(gt_occupancy_map == 0,
                                         cfg.FE.COLLISION_VAL,
        ...
        self.episodes_list = np.load(
            f'output/point_goal_episodes/{self.split}/{env_scene}.npy',
            allow_pickle=True)
    ...
    def write_to_file(self, num_samples=100):
        ...
        for idx_epi in range(len(self.episodes_list)):
            ...
            goal_coord = pose_to_coords((goal_pose[0], -goal_pose[1]),
                                        self.pose_range, self.coords_range,
                                        self.WH)
            # check if the start point is navigable
            if (not self.env.is_navigable(start_pose)) or (
                    not self.env.is_navigable(goal_pose)):
            ...
            try:
                ...
                semMap_module = SemanticMap(
                    self.split, self.scene_name, self.pose_range,
                ...
                if cfg.NAVI.HFOV == 90:
                    obs_list, pose_list = [], []
                    heading_angle = phi
                    obs, pose = get_obs_and_pose(self.env, start_pose,
                                                 heading_angle)
                    obs_list.append(obs)
                ...
                while step < cfg.NAVI.NUM_STEPS:
                    ...
                    agent_map_coords = pose_to_coords(agent_map_pose,
                                                      self.pose_range,
                                                      self.coords_range,
                                                      self.WH)
                    ...
                    if MODE_FIND_SUBGOAL:
                        ...
                        #============================== find the nearest frontier ==========================
                            ...
                            if lottery > cfg.PRED.PARTIAL_MAP.SAVING_GAP_PROB:
                                ...
                                M_p = np.stack((observed_occupancy_map,
                                                built_semantic_map))
                                U_PS = np.zeros((self.H, self.W),
                                                dtype=np.int16)
                                U_RS = np.zeros((self.H, self.W),
                                                dtype=np.float32)
                                U_RE = np.zeros((self.H, self.W),
                                                dtype=np.float32)
                                mask_PS = np.zeros((self.H, self.W),
                                                   dtype=bool)
                                mask_RS = np.zeros((self.H, self.W),
                                                   dtype=bool)
                                mask_RE = np.zeros((self.H, self.W),
                                                   dtype=bool)
                                q_G = np.zeros((2, self.H, self.W),
                                               dtype=np.int16)
                                ...
                                for fron in frontiers:
                                    ...
                                    # for P_S
                                    U_PS[points[:, 0],
                                         points[:, 1]] = int(1. * fron.P_S)
                                    mask_PS[points[:, 0], points[:, 1]] = True
                                    if fron.P_S > 0:
                                        # for R_S
                                        U_RS[points[:, 0],
                                             points[:, 1]] = fron.R_S
                                        mask_RS[points[:, 0], points[:,
                                                                     1]] = True
                                    else:
                                        # for R_E
                                        U_RE[points[:, 0],
                                             points[:, 1]] = fron.R_E
                                        mask_RE[points[:, 0], points[:,
                                                                     1]] = True
                                    # for goal map
                                    q_G[0, points[:, 0],
                                        points[:, 1]] = goal_coord[0] - int(
                                            fron.centroid[1])
                                    q_G[1, points[:, 0],
                                ...
                                if self.split == 'train':
                                    ...
                                    tform_trans = torch.Tensor([[
                                        agent_map_coords[0] - Wby2,
                                        agent_map_coords[1] - Hby2, 0
                                    ]])
                                    crop_center = torch.Tensor([[
                                        W / 2.0, H / 2.0
                                    ]]) + tform_trans[:, :2]
                                    map_size = int(
                                        cfg.PRED.PARTIAL_MAP.OUTPUT_MAP_SIZE /
                                        cfg.SEM_MAP.CELL_SIZE)
                                    tensor_M_p = crop_map(
                                        tensor_M_p, crop_center, map_size,
                                        'nearest')
                                    tensor_U_PS = crop_map(
                                        tensor_U_PS, crop_center, map_size,
                                        'nearest')
                                    tensor_U_RS = crop_map(
                                        tensor_U_RS, crop_center, map_size,
                                        'nearest')
                                    tensor_U_RE = crop_map(
                                        tensor_U_RE, crop_center, map_size,
                                        'nearest')
                                    tensor_mask_PS = crop_map(
                                        tensor_mask_PS, crop_center, map_size,
                                        'nearest')
                                    tensor_mask_RS = crop_map(
                                        tensor_mask_RS, crop_center, map_size,
                                        'nearest')
                                    tensor_mask_RE = crop_map(
                                        tensor_mask_RE, crop_center, map_size,
                                        'nearest')
                                    tensor_q_G = crop_map(
                                        tensor_q_G, crop_center, map_size,
                                        'nearest')
                                elif self.split == 'val':
                                    _, H, W = M_p.shape
                                    Wby2, Hby2 = W // 2, H // 2
                                ...
                                M_p = tensor_M_p.squeeze(0).numpy().astype(
                                    np.int16)
                                ...
                                U_RE = tensor_U_RE.squeeze(0).squeeze(
                                    0).numpy().astype(np.float32)
                                mask_PS = tensor_mask_PS.squeeze(0).squeeze(
                                    0).numpy().astype(bool)
                                mask_RS = tensor_mask_RS.squeeze(0).squeeze(
                                    0).numpy().astype(bool)
                                ...
                                with bz2.BZ2File(
                                        f'{self.scene_folder}/{sample_name}.pbz2',
                                        'w') as fp:
                    ...
                    if cfg.NAVI.FLAG_VISUALIZE_MIDDLE_TRAJ:
                        ...
                        ax.scatter(x_coord_lst,
                                   ...
                                   cmap='viridis',
                                   s=np.linspace(5, 2,
                                                 num=len(x_coord_lst))**2,
                                   zorder=3)
                        if not MODE_FIND_GOAL:
                            for f in frontiers:
                                ax.scatter(f.points[1],
                                           f.points[0],
                                           c='yellow',
                                           zorder=2)
                                ax.scatter(f.centroid[1],
                                           f.centroid[0],
                                           c='red',
                                           zorder=2)
...
def main():
    ...
    if cfg.PRED.PARTIAL_MAP.multiprocessing == 'single':  # single process
        for scene in scene_list:
            ...
            gen.write_to_file(num_samples=cfg.PRED.PARTIAL_MAP.
                              NUM_GENERATED_SAMPLES_PER_SCENE)
    elif cfg.PRED.PARTIAL_MAP.multiprocessing == 'mp':
        with multiprocessing.Pool(
                processes=cfg.PRED.PARTIAL_MAP.NUM_PROCESS) as pool:
