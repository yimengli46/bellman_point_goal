...
def nav_test(env_scene, output_folder, scene_floor_dict):
    ...
    for floor_id in list(scene_dict.keys()):
        ...
        print(
            f'**********scene_name = {scene_name}, device_id ={device_id}***********'
        )
        scene_output_folder = f'{output_folder}/{scene_name}'
        create_folder(scene_output_folder)
        ...
        for idx, data in enumerate(testing_data):
            ...
            try:
                steps, trajectory, action_lst, nav_metrics = nav_DP(
                    split, env, idx, scene_name, height, start_pose, goal_pose,
                    start_goal_geodesic_distance, saved_folder, device)
            except:
                print(
                    f'CCCCCCCCCCCCCC failed {scene_name} EPS {idx} DDDDDDDDDDDDDDD'
                )
...
def main():
    ...
    with multiprocessing.Pool(processes=len(args0),
