...
#'''
env_scene = 'YFuZgdQ5vWj' 
floor_id = 2
scene_name = 'YFuZgdQ5vWj_2' 
data_idx = 12
saved_folder = f'output/TESTING_RESULTS_Frontier/DP_NAVMESH'
#'''

'''
env_scene = '2t7WUuJeko7' 
floor_id = 0
scene_name = '2t7WUuJeko7_0' 
data_idx = 26
saved_folder = f'output/TESTING_RESULTS_Frontier/DP_PCDHEIGHT'
'''
...
scene_name = '2t7WUuJeko7_0' 
data_idx = 26
saved_folder = f'output/TESTING_RESULTS_Frontier/Optimum_PCDHEIGHT'
'''

create_folder(saved_folder)

scene_floor_dict = np.load(f'{cfg.GENERAL.SCENE_HEIGHTS_DICT_PATH}/large_scale_{split}_scene_floor_dict.npy',
	allow_pickle=True).item()

results_step_npy = np.load(f'output/LARGE_TESTING_RESULTS_90degree_DP_NAVMESH_MAP_UNet_OCCandSEM_Potential_1STEP_500STEPS/results_{scene_name}.npy', allow_pickle=True).item()
#results_step_npy = np.load(f'output/LARGE_TESTING_RESULTS_90degree_DP_PCDHEIGHT_MAP_UNet_OCCandSEM_Potential_1STEP_500STEPS/results_{scene_name}.npy', allow_pickle=True).item()
#results_step_npy = np.load(f'output/LARGE_TESTING_RESULTS_90degree_Optimistic_PCDHEIGHT_MAP_1STEP_500STEPS/results_{scene_name}.npy', allow_pickle=True).item()
saved_actions = results_step_npy[data_idx]['actions']

cfg.merge_from_file('configs/large_exp_90degree_DP_NAVMESH_MAP_UNet_OCCandSEM_Potential_1STEP_500STEPS.yaml')
#cfg.merge_from_file('configs/large_exp_90degree_DP_PCDHEIGHT_MAP_UNet_OCCandSEM_Potential_1STEP_500STEPS.yaml')
...
start_pose, goal_pose, _ = scene_floor_dict[env_scene][floor_id]['start_goal_pair'][data_idx]
...
occ_map_npy = np.load(f'output/large_scale_semantic_map/{scene_name}/BEV_occupancy_map.npy', allow_pickle=True).item()
...
while step < cfg.NAVI.NUM_STEPS:
		...
		#============================== find the nearest frontier ==========================
			...
			if cfg.NAVI.PERCEPTION == 'UNet_Potential':
				frontiers = fr_utils.compute_frontier_potential(
					...
					unet_model, device, LN, agent_map_pose)
			...
			if cfg.NAVI.STRATEGY == 'Optimistic':
				chosen_frontier = fr_utils.get_frontier_nearest_to_goal(
					agent_map_pose, new_frontiers, goal_coord, LN,
					observed_occupancy_map)
			elif cfg.NAVI.STRATEGY == 'DP':
				top_frontiers = fr_utils.select_top_frontiers(
					new_frontiers, top_n=8)
	...
	if True:
		...
		if not MODE_FIND_GOAL:
			...
			if chosen_frontier is not None:
				...
				print(f'chosen_frontier is None.')
			#'''
	...
	if act == -1 or act == 0: # finished navigating to the subgoal
		...
		if cfg.NAVI.HFOV == 90:
			...
			next_pose = pose
			agent_pos = np.array(
				[next_pose[0], scene_height, next_pose[1]])
			for rot in [90, 180, 270, 0]:
				heading_angle = rot / 180 * np.pi
				heading_angle = plus_theta_fn(heading_angle,
											  -next_pose[2])
				obs, pose = get_obs_and_pose(env, agent_pos,
											 heading_angle)
				obs_list.append(obs)
				pose_list.append(pose)
