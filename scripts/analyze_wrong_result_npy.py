...
cfg.merge_from_file(
    f'configs/large_exp_90degree_Optimistic_PCDHEIGHT_MAP_1STEP_500STEPS.yaml')
cfg.freeze()
...
for env_scene in cfg.MAIN.TEST_SCENE_NO_FLOOR_LIST:
    ...
    for floor_id in list(scene_dict.keys()):
        ...
        try:
            results_npy = np.load(
                f'{output_folder}/{result_folder}/results_{scene_name}.npy',
                allow_pickle=True).item()
            ...
            for i in range(num_test):
                ...
                start_pose, goal_pose, start_end_episode_distance = testing_data[
                    i]
                ...
                if 'success' in result['nav_metrics']:
                    ...
                    if metrics_success > 0.:
                        ...
                        spl = success * (start_end_episode_distance /
                                         max(start_end_episode_distance,
                    ...
                    df = df.append(
                        {
                            ...
                            'SPL': metrics_spl,
                            'SoftSPL': metrics_softspl
                        },
                        ignore_index=True)

                    print(
                        f'scene_name = {scene_name}, eps = {eps_id}, Success = {metrics_success}, SPL = {metrics_spl}'
                    )
                else:
                    df = df.append(
                        {
...
scene_info = pd.concat([scene_success, scene_spl, scene_softspl],
                       axis='columns',
                       sort=False)
