...
class RolloutState(object):
    ...
    """
    def __init__(self, subgoal_props, distances, frontier_idx, old_state=None):
...
def get_lowest_cost_ordering(subgoal_props, distances):
...
def get_best_expected_cost_and_frontier_list(grid,
    ...
    # Fix the 'is_subgoal' vector for the pruned subgoal before generating 
    # the rollout histories
    vertex_count = len(gcn_graph_input['is_subgoal'])
    new_is_subgoal = [0] * vertex_count
    ...
    for f in frontiers:
        ...
        new_is_subgoal[index] = 1
    gcn_graph_input['is_subgoal'] = new_is_subgoal

    # Estimate properties of all subgoals for all history rollout histories
    new_data = {}
    # Need to calculate the histories first
    all_history = lsp_cond.utils. \
        generate_all_rollout_history(gcn_graph_input['is_subgoal'])
    ...
    for history in all_history:
        ...
        new_data = lsp_cond.core.get_new_conditional_data(
            subgoals=frontiers,
            new_data=new_data,
            history=history_for_new_data_key
        )
    conditional_out = lsp_cond.core.get_lowest_cost_ordering(new_data, distances)
